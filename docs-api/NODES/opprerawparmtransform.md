...
`matrix  opprerawparmtransform(int opid, int trsorder, int xyzorder, int
mask)`
...
原始参数变换是由变换参数建立的，不包括CHOP IK求解器的效果。
