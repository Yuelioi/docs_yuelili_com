...
不要使用这个函数。file.hinclude文件有这个函数的更方便的版本，它返回一个结构。
...
然后使用该文件中的file_statorcached_file_statfunctions。它们接受一个文件路径字符串并返回一个结构体（定义在file.h中），其成员如下。  
