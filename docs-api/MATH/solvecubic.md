...
`int  solvecubic(float a, float b, float c, float d, float &t1, float &t2,
float &t3)`

`int  solvecubic(float a, float b, float c, float d, vector2 &t1, vector2 &t2,
vector2 &t3)`
...
求解给定的三次函数，其中a,b,c和da是系数，因此：ax^3 + bx^2 + cx + d = 0
...
在实数情况下，返回的根数将以升序排列。在只有一个根的情况下，该根被填入ot1,t2和t3。

In the complex case t1, t2, and t3 are the complex roots.

在复数情况下，et1, t2, 和t3是复数根。
