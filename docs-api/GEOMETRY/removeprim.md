...
要写入的几何体的句柄。目前唯一有效的值是0orgeoself，也就是一个节点中的当前几何体。(将来可能会使用此参数，以允许向其它几何体写入。)
...
如果此值是1，函数还会删除与该基元关联的、未与任何其它基元关联的任何点。
...
如果有些基元被移除，而有些基元被移除，而点被设为1，那么点被设为0的所有基元将在点被设为1的所有基元之前被删除。
