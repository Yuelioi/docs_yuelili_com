...
创建一个没有任何点的多边形或多段线。然后可以使用addvertex 向基元添加顶点。
...
`void  addprim(int &prim_num, int geohandle, string type, int pt0, int pt1,
int &vertices[])`

`void  addprim(int &prim_num, int geohandle, string type, int pt0, int pt1,
int pt2, int &vertices[])`

`void  addprim(int &prim_num, int geohandle, string type, int pt0, int pt1,
int pt2, int pt3, int &vertices[])`

`void  addprim(int &prim_num, int geohandle, string type, int points[], int
&vertices[])`
...
这些签名用与给定的点相对应的新基元的顶点编号填充给定的verticesarray。您可以使用这些数字与setvertexattrib来设置顶点的属性，但它们可能不是顶点的最终数字。
...
要写入的几何体的句柄。目前唯一有效的值是0orgeoself，也就是一个节点中的当前几何体。(这个参数将来可能会被用来允许写到其它几何体。)
...
您可以使用setprimattrib的返回值来设置新点的属性，但它可能不是点的最终编号。
...
例如，您可以使用setprimintrinsic来设置一个基元的变换。
