...
It can registered to be called by After Effects to update the menu with `AEGP\_RegisterUpdateMenuHook()`, and it can dim/activate the menu item using `AEGP\_EnableCommand()`/`DisableCommand()`.
