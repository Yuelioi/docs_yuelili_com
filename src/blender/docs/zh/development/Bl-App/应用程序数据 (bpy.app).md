...
## 关于修改数据的说明 #
    ...
    如果处理程序改变了被视口访问的数据，可能导致Blender崩溃。此时，请在开始渲染前锁定接口（Render → Lock Interface 或bpy.types.RenderSettings.use_lock_interface）。
...
## translations.contexts_C_to_py #
    ...
    说明：一个只读的字典，将上下文的C-identifiers映射到它们的Py-identifiers。
