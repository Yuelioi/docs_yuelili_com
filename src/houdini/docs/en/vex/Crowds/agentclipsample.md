...
been loaded for an agent primitive."), `prim` is out of range, `prim` is not
an agent primitive, `channel_index` is out of range, or `channel` does not
exist.

For sampling the clip’s transform channels, use
[agentclipsamplelocal](agentclipsamplelocal.html "Samples an agent’s animation
clip at a specific time.") or [agentclipsampleworld](agentclipsampleworld.html "Samples an agent’s animation clip at a specific time.") instead.
...
the [clip’s length](agentcliplength.html) "Returns the length (in seconds) of
an agent’s animation clip."), it will be wrapped around.
...
```c
float value = agentclipsample(0, @primnum, "walk", 1.2,
"latch_leftfoot");
```
