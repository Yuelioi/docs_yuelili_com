...
flags like `PBR_BSDF_REVERSE` or `PBR_BSDF_O_EVENT_EXIT`.
...
```c
sample_bsdf(F, inI, outI, eval, type, sx, sy,  "direct", 0, // Specify
indirect illumination "import:sssmfp", sssmfp, // Read the exported sssmfp
parameter ... );
```
