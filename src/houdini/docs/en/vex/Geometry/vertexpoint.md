...
The linear vertex number. The `vertexindex` function can be used to compute a
linear vertex from a primitive number and vertex number pair.
...
```c
int pt;  // Get the point of vertex 3 pt = vertexpoint("defgeo.bgeo",
3);
```
