...
`matrix orthographic(float zoom, float orthowidth, float image\_aspect, float pixel\_aspect, float clip\_near, float clip\_far)`

`matrix orthographic(float zoom, float orthowidth, float image\_aspect, float pixel\_aspect, float clip\_near, float clip\_far, vector4 window)`

用给定的参数创建一个正射投影矩阵。这可以用来将点投射到摄像机的所谓 NDC 坐标上。

为了从世界空间到 NDC 空间进行一次转换，给定一个相机矩阵和一个投影矩阵，你可以使用，`worldToNDC = worldToCamera * projection;`。
...
图像的长宽比。有时 image_aspect 用`xres`和`yres`来表示。在这种情况下，`image_aspect = xres / yres'。
...
window.x 和 window.y 是投影窗口的最小 xy 坐标，window.z 和 window.w 是投影窗口的最大 xy 坐标。这个参数是可选的，如果没有给出，则默认为{0,0,1,1}。
