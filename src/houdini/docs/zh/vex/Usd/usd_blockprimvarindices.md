...
`int usd\_blockprimvarindices(int stagehandle, string primpath, string name)`

This function blocks the primvar indices. I.e., removes all time samples and sets the _block_ as default value. It changes the indexed primvar into a non-indexed primvar.

::: info Note, you may also want to block the primvar itself with [usd_blockprimvar](usd_blockprimvar.html "Blocks the primvar.").
...
usd\_blockprimvarindices(0, "/geo/sphere", "primvar\_name");
