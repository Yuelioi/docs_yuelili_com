...
`int usd\_addscale(int stagehandle, string primpath, string suffix, vector scale)`

此函数将比例尺应用于基元。它创建并设置了一个定义比例的变换操作属性的值，并将其附加到基元的变换顺序。
...
```c
// Scale the cube
usd\_addscale(0, "/geo/cube", "my\_scale", {0.25, 0.5, 2});
