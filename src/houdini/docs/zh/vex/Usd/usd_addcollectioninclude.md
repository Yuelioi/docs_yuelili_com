...
`int usd\_addcollectioninclude(int stagehandle, string collectionpath, string path)`

这个函数将该对象包含在集合中。这通常是通过在集合的包含列表中添加一个明确的路径来实现的，但是如果足够的话，它也可以直接从集合的排除列表中删除一个路径。
...
usd\_addcollectioninclude(0, collection\_path, "/geo/sphere4");
