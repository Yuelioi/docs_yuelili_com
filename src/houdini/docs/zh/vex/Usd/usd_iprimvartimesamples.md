...
float[] usd\_iprimvartimesamplevalues(const int input; const string primpath, primvarname)
{
 float result[];

 float time\_samples[] = usd\_iprimvartimesamples( input, primpath, primvarname );
 foreach( float time\_code ; time\_samples )
 {
 float value = usd\_iprimvar( input, primpath, primvarname, time\_code );
 push( result, value );
 }
