...
`int usd\_blockprimvar(int stagehandle, string primpath, string name)`

此函数封锁了 primvar。即删除所有时间样本并将*block*设置为默认值。

::: info Note, if primvar is indexed, you may also want to block the indices with
[usd_blockprimvarindices](usd_blockprimvarindices.html) () ("阻断 primvar。")。
...
```c
// Block the primvar.
usd\_blockprimvar(0, "/geo/sphere", "primvar\_name");
