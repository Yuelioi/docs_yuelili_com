...
`float [] usd\_primvartimesamples(<stage>stage, string primpath, string name)`
...
float time\_codes[] = usd\_primvartimesamples(0, "/geo/cube", "foo");

```

```c
// Get primvar values at authored time samples on the given primitive.
float[] usd\_primvartimesamplevalues(const int input; const string primpath, primvarname)
{
 float result[];

 float time\_samples[] = usd\_primvartimesamples( input, primpath, primvarname );
 foreach( float time\_code ; time\_samples )
 {
 float value = usd\_primvar( input, primpath, primvarname, time\_code );
 push( result, value );
 }
