...
`void  split_bsdf(bsdf &lobes[], bsdf source, float &weights[], int mask, int
type)`
...
该函数用分割后的裂片的权重填充该数组（与返回的bsdfarray长度相同）。当你使用返回的裂片进行照明采样时，你必须用这些权重进行缩放。
