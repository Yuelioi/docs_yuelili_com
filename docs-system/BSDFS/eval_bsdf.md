...
`vector  eval_bsdf(bsdf b, vector viewer, vector light, vector normal, int
mask, ...)`

`vector  eval_bsdf(bsdf b, vector viewer, vector light, vector normal, float
&pdf, int mask, ...)`
...
该函数用给定方向的计算的PDF覆盖这个变量，并以反照率进行缩放。
...
eval_bsdffunction将任何额外的 "名称"、值参数对传递给正在评估的BSDF。
