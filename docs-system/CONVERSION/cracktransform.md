...
## 描述
angles in degrees to a vector of angles in radians.For example: `vector angles
= radians(cracktransform(XFORM_TRS, XFORM_XYZ, 1, {0,0,0}, M));`

(c=1或c=4)、缩放(c=2)或剪切(c=3)的变换分量（xform）。该

Note

Rotation angles (when `c=4`) are returned in radians.

函数使用给定的变换和旋转命令（trs和xyz），给定的支点（pivot）和可选的支点旋转（pr）来计算返回值。

`void  cracktransform(int trs, int xyz, vector pivot, vector pivot_rotate,
matrix xform, vector &t, vector &r, vector &s, vector &shears)`
...
`void  cracktransform(int trs, int xyz, vector pivot, matrix xform, vector &t,
vector &r, vector &s)`
...
在$HFS/houdini/vex/include/math.h中找到。
