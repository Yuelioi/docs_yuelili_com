...
`int [] pcfind_radius(<geometry>geometry, string Pchannel, string RadChannel,
float radscale, vector P, float radius, int maxpoints)`

`int [] pcfind_radius(<geometry>geometry, string ptgroup, string Pchannel,
string RadChannel, float radscale, vector P, float radius, int maxpoints)`

`int [] pcfind_radius(<geometry>geometry, string Pchannel, string RadChannel,
float radscale, vector P, float radius, int maxpoints, float &distances[])`
...
的点的列表，该列表基于在Pchannel中找到的点的位置。 每一个点都会被它们的RadChannel属性所扩展，并以yradscale进行扩张。
...
正因为如此，使用0.0radius也是明智的，该函数只需找到你的查询位置所处的所有源球体。
...
的最接近的点才会被返回。文件名可以使用op:语法来引用SOP
...
pgroup是一个点组，用来限制要搜索的点。 这是一个SOP风格的组模式，所以可以是like0-10or@Cd.x>0.5。
一个空白的字符串被视为匹配所有的点。
