...
如果channel_name不是可读可写的，或者如果（在采取半径的pcexport版本中）被导出的点在一个半径上，那么导出将失败。
...
这个函数写到用pcopenorpcgenerate打开的点的通道。这个函数的第二个版本接受一个半径参数，并根据它与点云中已存在的点的距离来接受或拒绝被导出的点。
它必须与其他所有的点相隔至少有指定的半径。 要将新的点数据写进点云文件，请使用epcwrite。
