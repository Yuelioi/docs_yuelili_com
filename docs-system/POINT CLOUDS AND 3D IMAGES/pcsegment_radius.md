...
`int [] pcsegment_radius(<geometry>geometry, string PChannel, string
RadChannel, float radscale, vector P0, vector P1, float max_distance, int
maxpoints)`

`int [] pcsegment_radius(<geometry>geometry, string ptgroup, string PChannel,
string RadChannel, float radscale, vector P0, vector P1, float max_distance,
int maxpoints)`

`int [] pcsegment_radius(<geometry>geometry, string PChannel, string
RadChannel, float radscale, vector P0, vector P1, float max_distance, int
maxpoints, float &distances[])`
...
这些函数打开一个几何文件，并返回一个在从P0到P1的线段的max_distance内的点（被视为球体）列表。每个点都被当作半径等于其RadChannel属性的球体。
