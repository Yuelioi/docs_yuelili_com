...
中包含的点。 然后你可以用pcunshaded或citerate来迭代这些点，并使用pcexport将新数据添加到点云中。
...
虽然这个功能类似于copen，但主要的区别是它排队的点可能是整个点群的集合。 换句话说
...
`distance` mode is provided for compatibility with [pcopen](pcopen.html
"Returns a handle to a point cloud file.") and doesnot queue up aggregate
...
如同在pcopen中，P指定了查询原点，Pchannel指定了位置通道。 在构建过程中，树状结构
...
点云中的所有点，并被递归细分，直到一个节点中的点少于min_ptspoints--此时，细分停止，一个叶子
