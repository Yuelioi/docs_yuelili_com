...
`int  addvertexattrib(int geohandle, string name, <type>defvalue, string
typeinfo)`
...
用给定的变换信息添加一个顶点属性。更多细节见attribtypeinfof。
...
要写入的几何体的句柄。目前唯一有效的值是0orgeoself，也就是一个节点中的当前几何体。(这个参数将来可能会被用来允许写到其他几何体。)
