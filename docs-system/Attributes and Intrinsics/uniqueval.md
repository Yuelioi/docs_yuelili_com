...
`int|string uniqueval(<geometry>geometry, string attribclass, string
...
如果几何体中的任何点/基元/顶点都有相同的给定属性值，那么uniquevalues的集合将小于点/基元/顶点的总数。这个函数可以让你遍历唯一值的集合。
...
Which one of the unique values to return.Use [nuniqueval](nuniqueval.html
"Returns the number of unique values from an integer or string attribute.") to
