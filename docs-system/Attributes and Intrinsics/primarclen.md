...
`float  primarclen(<geometry>geometry, vector2 uv1, vector2 uv2, int
prim_num)`
...
`float  primarclen(<geometry>geometry, vector2 uv1, vector2 uv2, int prim_num,
int divs, int primuvmode)`

`float  primarclen(<geometry>geometry, vector2 uv1, vector2 uv2, int prim_num,
int divs, int primuvmode, float primuvtol)`
...
定义 uv1 和 uv2 坐标单位。模式列表见eprimuvconvert。
...
你也可以读取arclengthprimitive的内在属性来取得曲线的总弧长。
