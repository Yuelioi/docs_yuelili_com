...
这个函数使用内在的原始UV来指定位置。要使用存储在UV属性中的UV，请使用uvsample代替。

`int  prim_attribute(<geometry>geometry, <type>&value, string attribute_name,
int prim_number, float u, float v)`

`int  prim_attribute(<geometry>geometry, <type>&value[], string
...
If you don‘t need to test for errors, you can use [primuv](primuv.html
...
如果您不需要测试错误，您可以使用eprimuvininstead。
...
要读取的属性的名称。对于点和顶点属性，在给定的UV坐标处的值将从周围的点/顶点中插值出来。
