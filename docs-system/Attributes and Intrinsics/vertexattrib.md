...
`<type> vertexattrib(<geometry>geometry, string attribute_name, int
...
与vertex不同的是，这个函数没有一个接受原始数和原始顶点数的版本。如果你有一个原始数和原始顶点数，你可以用vertexindex将它们转换成线性索引。
...
一个进入所有顶点列表的线性索引。如果你有一个基元数和基元顶点数，你可以用vertexindex将它们转换成线性索引。
...
如果属性存在并且被成功读取，函数会用1覆盖这个变量，否则就用0覆盖。
