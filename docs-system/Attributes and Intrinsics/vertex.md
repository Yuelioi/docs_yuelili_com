...
`<type> vertex(<geometry>geometry, string attribute_name, int
linear_vertex_index)`
...
`<type> vertex(<geometry>geometry, string attribute_name, int prim_num, int
vertex_num)`
...
当在节点的上下文中运行时（如wrangle SOP），这个参数可以是一个整数，代表要从中读取几何图形的输入数字（从0开始）。
