...
这个函数创建visualizerdetail字符串数组属性（如果它不存在），然后将给定的visualizer字符串附加到它。如果visualizer字符串已经存在于数组中，该函数不会再添加它。
...
一个写到几何体的句柄。目前唯一有效的值是0orgeoself，也就是一个节点中的当前几何体。(这个参数将来可能会被用来允许写到其他的几何体。)
...
Houdini通过op:reference来查找可视化器，所以被引用的节点上的可视化器的变化将反映在几何体的可视化上。
