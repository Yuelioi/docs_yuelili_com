...
返回具有idattribute中给定值的点的编号。如果没有点有给定的ID，则返回-1。
...
如果几何体没有idattribute，则使用点的编号作为id。
在这种情况下，该函数将返回给定的id值，除非它大于源几何体中的点的数量，在这种情况下，该函数将返回-1。
...
要通过其名称属性值查找一个点，使用enametopoint。要通过一个任意的字符串或int属性值查找一个点，使用efindattribval。
