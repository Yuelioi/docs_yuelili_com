...
`vector2  primuvconvert(<geometry>geometry, vector2 uv, int prim_num, int
mode)`
...
一个字符串，指定一个要读取的几何文件（例如，a.bgeo）。当在Houdini中运行时，这可以是anop:/path/to/sopreference。
...
要转换的曲线坐标。可以是一个浮点数或一个向量2。该函数返回转换后的坐标。
...
下面列出的PRIMUV_space_TO_space常量之一。你可以从$HFS/houdini/vex/include/math.h导入这些常数。
...
REAL域是基于曲线段的数量（0吨Segments）。UNITdomain是归一化的REAL域，适合于0到1.UNITLENdomain根据曲线的长度来映射曲线，但要归一化（0...1）.LENdomain根据曲线的长度来映射（0...CurveLength）。
