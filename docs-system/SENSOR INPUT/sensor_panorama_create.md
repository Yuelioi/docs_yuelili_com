...
`int  sensor_panorama_create(float time, vector pos, int size, float near,
float far, string candidateobj, string includeobj, string excludeobj, int
uselit)`
...
这个函数将使用GL渲染器来渲染周围的环境，并提供一个用于查询结果的句柄。
...
因为这需要渲染场景，所以它只在Houdini的交互式会话中工作。
