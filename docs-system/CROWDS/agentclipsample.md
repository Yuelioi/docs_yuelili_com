...
`float  agentclipsample(<geometry>geometry, int prim, string clipname, float
...
clips之一，prim超出范围，prim不是agent的primitive，channel_index超出范围，或者channeldo不存在，则返回0。
...
对于剪辑的变换通道的采样，使用agentclipsamplelocal或agentclipsampleworld来代替。
...
动画片断中一个通道的索引，由agentclipchannel返回。
