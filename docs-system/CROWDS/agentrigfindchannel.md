...
如果在装备中没有找到通道名称，或者基元超出范围，或者基元不是一个代理基元，则返回1。
...
这个索引可用于使用agentchannelvalue和setagentchannelvalue访问代理的当前通道值，或使用agentclipsample从任何片段中采样通道值。
...
当在一个节点的上下文中运行时（比如一个wrangle SOP），这个参数可以是一个整数，代表要读取几何图形的输入数字（从0开始）。
