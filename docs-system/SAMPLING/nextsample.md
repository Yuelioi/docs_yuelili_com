...
newsampler和这个函数揭示了mantra用于像素抗锯齿的高质量确定性采样模式。在光线追踪模式下进行渲染时，通过用SID全局变量初始化采样序列，可以用nextsampleroutine生成确定性的2D采样。
...
这种方法可以生成2D或3D采样模式。 要生成二维样本，请使用带有2个浮点只写参数的签名。 要生成三维样本，请使用带有一个矢量只写参数的签名。
...
推进到模式中的下一个样本。你应该在使用newsampler时使用这个模式。
...
推进到一个新的像素采样模式。当使用SID和光线追踪在一个像素内产生高质量的采样模式时，你应该使用这种模式。这种模式会考虑到当前像素内的其他样本，并且会适当地进行分层。
...
"类似，当与SID和光线追踪一起使用时，这种模式可以保留高质量的像素采样。
