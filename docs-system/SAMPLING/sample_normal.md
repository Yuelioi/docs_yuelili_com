...
`float  sample_normal(float origmean, float origstddev, float minvalue, float
maxvalue, float u)`
...
分布的平均值，如果它没有形成invalue和maxvalue，限制了范围的话。
...
分布的标准差（规模），如果它没有形成invalue和maxvalue，那么它就会有
...
的标准差，如果没有形成invalue和maxvalue，限制了范围。
