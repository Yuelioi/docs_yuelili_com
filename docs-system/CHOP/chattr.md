...
`<type> chattr(int opinput, string attrname, int channel, int sample, int
&success)`
...
`<type> chattr(string attrclass, string attrname, int channel, int sample, int
&success)`

`<type> chattr(int opinput, string attribclass, string attrname, int channel,
int sample, int &success)`

Get the value of an attribute of a specific class.The version without
`opinput` assumes the first input (0).

没有opinput的版本假设是第一个输入（0）。
...
CHOP属性存储片段、通道、采样或通道/采样对的元数据。

`"sample"`

An attribute on a sample (across all channels).

这个函数读取一个CHOP属性的值。使用chsetattr来设置CHOP属性。
