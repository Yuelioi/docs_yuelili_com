...
`vector getpointbbox\_center(<geometry>geometry)`

`vector getpointbbox\_center(<geometry>geometry, string pointgroup)`

这与 [getbbox_center](getbbox_center.html) () ("返回几何体的包围盒的中心。") 相同，只是它只计算\_点的包围盒。因此，如果一个基元有没有点的外延（例如，基元球的边界），它们就不会被包括在框内。
