...
如果`clipname`不是代理的[animation clips](agentclipcatalog.html) () ("返回一个代理基元已经加载的所有动画片段。")，`prim`超出范围，或者`prim`不是一个代理基元，则返回一个空数组。

要获得代理的变换列表，请使用[agenttransformnames](agenttransformnames.html) () ("返回代理基元中每个变换的名称。")。
