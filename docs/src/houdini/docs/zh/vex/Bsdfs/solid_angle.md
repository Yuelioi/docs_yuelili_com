...
`float solid\_angle(bsdf b, int mask)`
...
{
 angles[i] = solid\_angle(lobes[i], PBR\_ALL\_MASK);
}

// Compute PDF from angles
float pdf[] = compute\_pdf(angles);

// Compute CDF from PDF
float cdf[] = compute\_cdf(pdf);

// Randomly select a BSDF based on albedo distribution
int id = sample\_cdf(cdf, sx);
