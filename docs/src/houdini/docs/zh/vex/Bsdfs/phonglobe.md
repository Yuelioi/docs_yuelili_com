...
![](/images/phonglobe.png)

沿着一个给定的方向向量进行 Phong（模糊）反射。当方向向量是反射向量时，这将产生与`phong()`相同的结果，但用这个函数你也可以从其他方向收集照明（如透射）。

通过提供 x 和 y 指数和切线矢量，可以创建各向异性的峰状裂片。
...
沿着 "framex "向量的 phong 指数。

`exponenty`

沿着 "framey "向量的 phong 指数。
