...
`int usd\_clearmetadata(int stagehandle, string path, string name)`
...
该名称可以被命名为可访问的值，在（可能是嵌套的）VtDictionary 内，如自定义数据字典，例如 "customData:name "或 "customData:name:subname"。对于非命名空间的名称，对象模式需要声明一个给定的元数据，以便它能够被访问，例如，"活动 "或 "文档"。
...
usd\_clearmetadata(0, "/geo/sphere", "customData:some\_name");
