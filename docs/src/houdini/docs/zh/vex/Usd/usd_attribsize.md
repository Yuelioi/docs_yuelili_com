...
`int usd\_attribsize(<stage>stage, string primpath, string name)`

这个函数返回一个给定属性的元组大小。如果该属性是一个数组，它返回数组元素的元组大小。例如，对于向量类型，它是组件的数量。
...
如果想获得数组属性的长度，请使用[usd_attriblen](usd_attriblen.html)（"返回数组属性的长度。"）。
...
int tuple\_size = usd\_attribsize(0, "/geo/cube", "attribute\_name");
