...
`string usd\_findtransformname(<stage>stage, string primpath, string suffix)`
...
可能有几个具有相同后缀的变换操作（例如，一个平移和一个旋转），所以第一个遇到的将被返回。
...
string xform\_name = usd\_findtransformname(0, "/geo/cone", "cone\_pivot");
usd\_addinversetotransformorder(0, "/geo/cone", xform\_name);
