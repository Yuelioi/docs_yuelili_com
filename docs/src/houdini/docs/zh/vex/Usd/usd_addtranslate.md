...
`int usd\_addtranslate(int stagehandle, string primpath, string suffix, vector amount)`

此函数向基元应用了一个平移。它创建并设置了一个定义平移的转换操作属性的值，并将其附加到基元的转换顺序。
...
```c
// Translate the cube
usd\_addtranslate(0, "/geo/cube", "my\_translation", {10, 0, -2.5});
