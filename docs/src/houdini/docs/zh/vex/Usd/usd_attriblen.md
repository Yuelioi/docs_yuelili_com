...
`int usd\_attriblen(<stage>stage, string primpath, string name)`

`int usd\_attriblen(<stage>stage, string primpath, string name, float timecode)`
...
数组属性的长度，如果该属性不是一个数组，则为 1。如果想检查属性是否为数组，请使用[usd_isarray](usd_isarray.html) () ("Checks if the attribute is an array.") 。
...
int length = usd\_attriblen(0, "/geo/cube", "attribute\_name");
