...
`matrix optransform(int op\_id)`

`matrix optransform(int op\_id, float time)`

返回与一个 OP 相关的变换。如果指定的 OP 没有与之相关的变换（例如 COP），那么将返回一个相同的矩阵。可以指定评估变换的时间（单位是秒，而不是帧）。

注意：op: 语法可以用来模拟这种行为，使用标准转换函数。
