...
`matrix perspective(float zoom, float image\_aspect, float pixel\_aspect, float clip\_near, float clip\_far)`

`matrix perspective(float zoom, float image\_aspect, float pixel\_aspect, float clip\_near, float clip\_far, vector4 window)`

用给定的参数创建一个透视投影矩阵。这可以用来将点投射到摄像机的所谓 NDC 坐标上。
