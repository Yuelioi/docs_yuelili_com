...
[setpackedtransform](setpackedtransform.html) () ("设置一个打包的基元的变换。") 函数也有同样的问题，因为它_只覆盖`P`和`transform`。因此，例如，如果有一个非零的打包枢轴，或者在上面列出的其他情况下，`packedfulltransform`不会包含你期望的矩阵。
