...
返回半边的源点在几何体中的点号。`0'是第一个点。

`srcpoint`, `dstpoint`

返回的半边的源点和目的点在几何体中的编号。`0'是第一个点。

## Returns

以`point'为源头或以`srcpoint'为源头和`dstpoint'为目的地的半边的数量。在前一种情况下，使用`op:pointhedgenext`可以列举出所有以`point`为源的半边线。如果该半边线无效，则返回`-1'。
