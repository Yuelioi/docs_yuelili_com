...
`USD_XFORM_TRANSLATE`, `USD_XFORM_TRANSFORM`, or `USD_XFORM_ROTATE_XYZ`.

## Examples ¶

```c
// Get the type of the first transform operation on the cube string
cube_xform_ops[] = usd_transformorder(0, "/geo/cube"); int type =
usd_transformtype(cube_xform_ops[0]);
```
