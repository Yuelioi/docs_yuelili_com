...
orientation, and appends it to the primitive’s transform order.
...
```c
// Orient the cube vector4 quat = eulertoquaternion(radians({30,0,0}),
XFORM_XYZ); usd_addorient(0, "/dst/cone", "my_orientation", quat);
```
