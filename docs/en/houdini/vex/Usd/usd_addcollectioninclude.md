...
by adding an explicit path to the collection’s include list, but it may just
remove a path from the collection’s exclude list, if it’s sufficient.
...
```c
// Include sphere4 in cube's collection. string collection_path =
